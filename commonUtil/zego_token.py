...
from commonUtil import zego_config
...
def __make_random_iv():
    str_info = '07a41999a71b00a58f348481fae5c464b21467ec3f65d8b759e3a5f2531c120a'
    iv = ""
    for i in range(16):
...
def generate_token04(app_id, user_id, secret, effective_time_in_seconds, payload):
    ...
    Args:
        ...
        effective_time_in_seconds:  token的有效时长，单位：秒
        payload:有效载荷
...
def get_user_base_token(user_id):
    """
    获取基础鉴权token
    :param user_id: 用户ID
    :return:
    ...
    effective_time = zego_config.effective_time
    server_secret = zego_config.ServerSecret
    token = generate_token04(app_id, user_id, server_secret, effective_time, payload)
    return token.token
